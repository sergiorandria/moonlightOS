...
- `userspace/` - `mem_server` (color-aware), `sched_server` (EDF admission), `drivers/virtio_net` (IOMMU-isolated), `example/hello`, `lib/moonlight.h` purecap ABI
...
- Trap: `mtvec`/`mscratch` separate stacks, `mcause`+`mepc+4`, `sfence` - verified in QEMU `riscv64`/`riscv64cheristd`
