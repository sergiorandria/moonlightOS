...
General-purpose dynamic microkernel for **RISC-V CHERI**, partially verified in Isabelle/HOL (10/14 theorems proved, 4 axiomatized as known gaps, no `sorry` left except explicitly listed). **Production grade** - trap/paging/CHERI hybrid & purecap, QEMU window tested.
...
make -C kernel isabelle   # Isabelle2025-2 + l4v, 7 theories (no quick_and_dirty, 10 proved, 4 axiomatized)
...
## Proven Properties (10 proved, 4 axiomatized - see `docs/PRODUCTION.md` Verification)
...
- `Moonlight_A` `nonleakage_time`/`availability` proved, `integrity` axiomatized (IOMMU preservation), `Moonlight_E` `ex_refines_abs` axiomatized, `Refine` `refinement`/`c_refinement` axiomatized (AutoCorres/CompCert not set up)
