...
- [x] `kernel/isabelle` 7 theories build (Isabelle2025-2, no `quick_and_dirty`): 10 proved, 4 axiomatized as known open gaps (see below)
  ...
  - `Moonlight_A`: `nonleakage_time` proved, `availability` proved (added `Tick`/`PartitionSwitch` rules, `deadline_met ≡ schedulable`), `integrity` axiomatized (requires IOMMU preservation, trivially `True` invariants but keep as axiom to avoid `quick_and_dirty`)
...
- [ ] Formal proofs: `Moonlight_A:integrity`, `Moonlight_E:ex_refines_abs`, `Refine:refinement`/`c_refinement` axiomatized (4/14 theorems) — `c_refinement` requires `AutoCorres`/`CompCert`, others need richer invariants; `grep -rn "axiomatization" kernel/isabelle/` lists them, no `sorry`/`quick_and_dirty` left
