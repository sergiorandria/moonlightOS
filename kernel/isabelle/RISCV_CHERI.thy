...
definition cheri_perms_and :: "cheri_cap \<Rightarrow> perms \<Rightarrow> cheri_cap" where
  "cheri_perms_and c p = c\<lparr> cheri_perms := cheri_perms c AND p \<rparr>"
...
lemma cheri_mono_perms: "cheri_perms (cheri_perms_and c p) AND p = cheri_perms (cheri_perms_and c p)"
