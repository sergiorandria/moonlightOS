...
kerror_t cnode_init(cnode_t *cn, uint32_t guard, uint8_t radix) {
    if (!cn) return ERR_INVALID_ARG;
    if (radix > 8 || radix == 0) return ERR_INVALID_ARG;
...
kerror_t cnode_copy(cnode_t *dst, cptr_t dst_idx, cnode_t *src, cptr_t src_idx, uint32_t rights) {
    if (!dst || !src) return ERR_INVALID_ARG;
    if (dst_idx >= MAX_CAPS_PER_CNODE || src_idx >= MAX_CAPS_PER_CNODE) return ERR_INVALID_ARG;
    cap_t *sc = cnode_lookup(src, src_idx);
    if (!sc) return ERR_INVALID_CAP;
    if ((rights & ~sc->rights) != 0) return ERR_NO_RIGHTS;
    cap_t derived = cap_derive(*sc, rights, sc->badge);
    if (!derived.is_valid) return ERR_INVALID_CAP;
    if (dst->slots[dst_idx].is_valid) return ERR_REVOKE_NEEDED;
...
kerror_t cnode_mint(cnode_t *dst, cptr_t dst_idx, cnode_t *src, cptr_t src_idx, uint32_t rights, uint32_t badge) {
    ...
    cap_t *sc = cnode_lookup(src, src_idx);
    if (!sc) return ERR_INVALID_CAP;
    if (!cap_has_right(*sc, CAP_RIGHTS_GRANT)) return ERR_NO_RIGHTS;
    if (dst->slots[dst_idx].is_valid) return ERR_REVOKE_NEEDED;
...
kerror_t cnode_delete(cnode_t *cn, cptr_t idx) {
    if (!cn) return ERR_INVALID_ARG;
    if (idx >= MAX_CAPS_PER_CNODE) return ERR_INVALID_ARG;
    if (!cn->slots[idx].is_valid) return ERR_INVALID_CAP;
