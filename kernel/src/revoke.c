...
void mdb_init(mdb_tree_t *mdb) {
  ...
  for (uint32_t i = 0; i < MAX_MDB_NODES; i++)
    mdb->nodes[i].parent = NIL;
  mdb->free_head = 0;
  mdb->count = 0;
}

kerror_t mdb_insert(mdb_tree_t *mdb, uint32_t parent_idx, cptr_t cptr,
                    cap_t cap, uint32_t *out_idx) {
  if (!mdb || !out_idx)
    return ERR_INVALID_ARG;
  if (mdb->count >= MAX_MDB_NODES)
    return ERR_NO_MEM;
  uint32_t idx = mdb->free_head;
  while (idx < MAX_MDB_NODES && mdb->nodes[idx].valid)
    idx++;
  if (idx >= MAX_MDB_NODES)
    return ERR_NO_MEM;
  ...
  n->next_sibling = NIL;
  n->revocable = true;
  if (parent_idx != NIL && parent_idx < MAX_MDB_NODES &&
      mdb->nodes[parent_idx].valid) {
    n->next_sibling = mdb->nodes[parent_idx].first_child;
    mdb->nodes[parent_idx].first_child = idx;
  }
  mdb->count++;
  if (idx == mdb->free_head)
    mdb->free_head = idx + 1;
  *out_idx = idx;
  return ERR_OK;
}
...
kerror_t mdb_revoke(mdb_tree_t *mdb, uint32_t idx) {
  if (!mdb || idx >= MAX_MDB_NODES)
    return ERR_INVALID_ARG;
  if (!mdb->nodes[idx].valid)
  ...
  /* Keep parent node but clear its cap? No - revoke means delete subtree
   * excluding parent */
...
kerror_t mdb_delete(mdb_tree_t *mdb, uint32_t idx) {
  ...
  if (!mdb->nodes[idx].valid)
    return ERR_INVALID_CAP;
  if (mdb->nodes[idx].first_child != NIL)
...
uint32_t mdb_lookup(mdb_tree_t *mdb, uint32_t cnode_id, cptr_t slot) {
  for (uint32_t i = 0; i < MAX_MDB_NODES; i++) {
    if (!mdb->nodes[i].valid)
      continue;
    // cptr encodes cnode_id<<8 | slot, also check explicit fields
    if (mdb->nodes[i].cnode_id == cnode_id && mdb->nodes[i].slot == slot)
      return i;
    if (mdb->nodes[i].cptr == slot && cnode_id == 0)
      return i; // legacy: process uses cptr=tcb_id
    if (mdb->nodes[i].cptr == ((cnode_id << 8) | slot))
      return i;
  }
  return NIL;
}

bool mdb_is_descendant(mdb_tree_t *mdb, uint32_t ancestor,
                       uint32_t descendant) {
